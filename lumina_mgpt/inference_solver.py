...
from data.drafters.choices import *
...
class FlexARInferenceSolver:
    ...
    def __init__(self, model_path, precision, target_size=512, cache_dir=None, device="cpu", tokenizer="Alpha-VLLM/Lumina-mGPT-7B-768",local_chameleon_tokenizer_path="/data/txf/SJD/ckpts/chameleon"):
    ...
    def generate(
        ...
        _prompt = self.item_processor.process_item(item) # TODO: 分词器编码
        prompt = []
        ...
        with torch.cuda.amp.autocast(dtype=self.dtype):
            ...
            if return_accl:
                generation_result = result.input_ids[0][prompt_len:].tolist()
            else:
                generation_result = result[0][prompt_len:].tolist()
            if len(generation_result) > 0 and generation_result[-1] == 8710:
                generation_result = generation_result[:-1]

        if return_accl:
            result.input_ids = self.decode_ids(generation_result)
            return result
        else:
