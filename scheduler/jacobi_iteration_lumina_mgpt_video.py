...
def renew_sampler(model_class):
    ...
    class JacobiSampler(model_class, nn.Module):
        ...
        def _update_model_kwargs_for_frame_generation(
            self,
            outputs: ModelOutput,
            model_kwargs: Dict[str, Any],
            is_encoder_decoder: bool = False,
            ...
            if not is_encoder_decoder:
                ...
                if "attention_mask" in model_kwargs:
                    ...
                    while len(attention_mask.shape) < 3:
                        attention_mask = attention_mask.unsqueeze(1)
                    #TODO: 删除最新的那个字符 attention_mask = attention_mask[...,:last_position]
                    attention_mask = attention_mask[..., -1:, :last_position+1] #
            ...
            if model_kwargs.get("use_cache", True):
                ...
                #     model_kwargs["cache_position"] = model_kwargs["cache_position"][-1:] + num_new_tokens
                new_positions = torch.arange(
                    last_position + 1, last_position + num_new_tokens + 1, dtype=past_positions.dtype
                ).to(past_positions.device)
                model_kwargs["cache_position"] = new_positions
            else:
                new_positions = torch.arange(
            ...
            # 删掉不必要的kv cache
            past_key_values = model_kwargs["past_key_values"]
            if last_position + 1 < past_key_values.get_seq_length():
                remaining_tokens_num = past_key_values.get_seq_length() - (last_position + 1)
                delete_false_key_value(past_key_values, remaining_tokens_num)
            ...
            **model_kwargs,
            ...
            for frame_id in range(frame):
                while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device, cur_len=cur_len, max_length=max_length):
                    ...
                    judge_token = updated_input_ids[:,input_ids.shape[-1]:updated_input_ids.shape[-1]]
                    ...
                    continue_flag = True # 判断是否继续的符号
                    for i in range(judge_token.shape[-1]):# 有假则假, 如果里面有停止符stopping_criteria返回True, 则~stopping_criteria为false
                        continue_flag = continue_flag & ~stopping_criteria(judge_token[:,i][None], scores)
                    unfinished_sequences = unfinished_sequences & continue_flag# 只要continue_flag为false,就不继续了
                ...
                #TODO:在剩余的input_ids里拼接上frame_tokens,frame_split token是用于分割不同帧之间的区别
                frame_split = self.fsp_input_ids
                where_is_end = torch.nonzero(input_ids == generation_config.eos_token_id, as_tuple=True)[1]
                last_end_position = where_is_end[-1]
                input_ids = input_ids[:, :last_end_position]
                input_ids = torch.cat([input_ids,frame_split],axis=-1)
                #TODO: 重新地让其又能进入一次循环
                unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
                this_peer_finished = False
                output_token_num = 1
                additional_tokens = None
                additional_scores = None
                #TODO: 需要重新校对很多东西, 包括位置,KV_Cache信息,attention_mask信息,position信息
                # "补充代码" 'attention_mask' 'neg_input_ids' 'past_key_values' 'cache_position'
                model_kwargs = self._update_model_kwargs_for_frame_generation(
                        outputs,
                        model_kwargs,
                        is_encoder_decoder=self.config.is_encoder_decoder,
                        num_new_tokens = frame_split.shape[-1],
                        last_position = last_end_position-1
                    )
                del outputs
