...
def get_eol_in_multitokens_with_cache_position(logits, eol_cls, tokenlen, new_pred_tokenlen, line_len, cache_position, min_dtype=-math.inf):
    logits_forced_eol = logits.clone() # torch.argmax(logits[:,-3])
    L, R = (tokenlen + 1), (tokenlen + new_pred_tokenlen)
    check_interval_l = L // line_len + 1 if L % line_len != 0 else L // line_len
    check_interval_r = R // line_len
    ...
    cache_position_line = torch.unique(cache_position)
    for i in eol_position_ids:
        assert i < len(cache_position_line)
        matching_indices = torch.where(cache_position == cache_position_line[i])[0]
        # logits_forced_eol[..., i, :] = min_dtype
        # logits_forced_eol[..., i, eol_cls] = 0
        logits_forced_eol[..., matching_indices, :] = min_dtype
        logits_forced_eol[..., matching_indices, eol_cls] = 0
...
class MultiTreeTokensVLLogitsProcessor(LogitsProcessor):
        ...
        if self.num_image_start_tokens == self.num_image_end_tokens:
            ...
            if new_token_num >= 2:
                ...
                if self.cache_position is None:
                    ...
                    # containing ONE end-of-line
                    if is_new_seq_ids_containing_end_of_line:
                        scores, eol_position_ids = get_eol_in_multitokens_with_cache_position(
                            scores, self.image_next_line_token_id, len(tokens), 
                            new_logit_token_len, self.w_latent_dim + pad_eol_len, self.cache_position
                        )
                    ...
                    if is_new_seq_ids_containing_end_of_img:
                        scores, eol_position_ids = get_eol_in_multitokens_with_cache_position(
                            scores, self.image_end_token_id, len(tokens), 
                            new_logit_token_len, 
                            (self.w_latent_dim + pad_eol_len) * self.h_latent_dim + pad_eol_len, self.cache_position
                        )
                return scores
        else:
            print(f"Something wrong in the decoding process. MultiTokensVLLogitsProcessor. \
