...
from .tree_tool import *
from .tree_until import *
from .oneline_utils import SpeculativeSampler_PlusPlus
...
def sampling_logits2tokens(
    ...
    logits_processor[0].cache_position = cache_position
    next_token_scores = logits_processor(all_collected_input_ids, next_token_logits)
    logits_processor[0].cache_position = None
...
def get_draft_token_for_tree(
    img_vocab, 
    tree_input_ids, 
    input_ids, temporary_collected_scores, device, 
    multi_token_init_scheme=None,
):
    rand_token_num = (tree_input_ids == -1).sum().item()
    if multi_token_init_scheme == 'random':
        img_vocab = img_vocab.to(device)
        img_vocab_size = len(img_vocab)
        rand_tokens = torch.randint(
        ...
        # one_score = 1 / img_vocab_size
        one_score = 1.0
        scores_of_rand_tokens = torch.scatter(scores_of_rand_tokens, -1, rand_tokens.unsqueeze(-1), one_score)

    return rand_tokens, scores_of_rand_tokens

def renew_static_tree_sampler(model_class):
            ...
            **model_kwargs,
            ...
            if self.prefix_token_sampler_scheme == 'speculative_jacobi':
                ...
                prefix_token_sampler = SpeculativeSampler_PlusPlus(
                    generator=self.generator,
                    reject_sampling_relative_ids = -torch.ones(
                        100, dtype=dtype, device=device,
                    ),
                    reject_sampling_draft_token_logits = torch.zeros(
                        (100, self.config.vocab_size), dtype=dtype, device=device
                    )
                )
                prefix_token_sampler.threshold = generation_config.threshold if hasattr(generation_config, 'threshold') else 1.0
            else:
...
def renew_static_tree_sampler_plus(model_class):
    def pad_path(path: List[int], length: int, pad_value: int = -2) -> List[int]:
        """
        Pad the given path list with a specific value up to a specified length.
    ...
    class JacobiSampler(model_class, nn.Module):
        ...
        def _update_model_kwargs_for_generation(#需要修改为草稿树的样子
            self,
            outputs: ModelOutput,
            model_kwargs: Dict[str, Any],
            is_encoder_decoder: bool = False,
        ...
        def _sample(
            ...
            **model_kwargs,
            ...
            # # 计算平均值（保留两位小数）
            # total = sum(accept_list)
            # average = total / len(accept_list)
            # for num, count in sorted(element_count.items()):
            #     print(f"  整数 {num}: 出现 {count} 次")
            # print("=" * 50)
            # print(f"列表平均值: {round(average, 2)}")
            # print("=" * 50)
            # import torch.nn.utils.rnn as rnn
            # def positional_mean_padded(lst, padding_value=0.0):
            #     if not lst:
            #         return torch.tensor([])
            #     max_len = max(len(t) for t in lst)
            #     # 右填充到 max_len（batch_first=True）
            #     padded = rnn.pad_sequence(lst, batch_first=True, padding_value=padding_value)
            #     # 创建掩码：1=有效，0=padding
            #     mask = torch.arange(max_len)[None, :] < torch.tensor([len(t) for t in lst])[:, None]
            #     mask = mask.to(padded.device)
            #     # 安全平均：(padded * mask).sum(0) / mask.sum(0)，避免除0
            #     valid_counts = mask.sum(dim=0).clamp(min=1)  # 至少1避免 nan
            #     valid_counts = valid_counts.to(padded.device)
            #     means = (padded * mask.float()).sum(dim=0) / valid_counts
            #     return means
            # print(f"各位置平均接受概率: {positional_mean_padded(acrate_list)}")
            #---------------Return---------------
            if streamer is not None:
                streamer.end()
...
def renew_static_tree_backbone(model_class):
    class JacobiBackbone(model_class):
        ...
        def _update_causal_mask(
            ...
            if attention_mask is not None and attention_mask.dim() == 4:
                ...
                causal_mask = torch.full((sequence_length, target_length), fill_value=min_dtype, dtype=dtype, device=device)
                assert torch.unique(cache_position).shape[0] <= sequence_length
                if torch.unique(cache_position).shape[0] < sequence_length:
                    # Tree mask
                    if sequence_length != 1:
                        causal_tree_mask = 1 - attention_mask[0]
                        causal_mask *= causal_tree_mask
                else:
                    if sequence_length != 1:
                        causal_mask = torch.triu(causal_mask, diagonal=1)
                    causal_mask *= torch.arange(target_length, device=device) > cache_position.reshape(-1, 1)
                causal_mask = causal_mask[None, None, :, :].expand(input_tensor.shape[0], 1, -1, -1)
                if attention_mask is not None: # 没什么用, 因为padding_mask不会有0
                    causal_mask = causal_mask.clone()  # copy to contiguous memory for in-place edit
                    mask_length = attention_mask.shape[-1]
    ...
    return JacobiBackbone
# for base forward
from .old_jacobi_iteration_lumina_mgpt import renew_sampler
from .jacobi_iteration_lumina_mgpt_sjd_pp import renew_sampler as sjd_pp_sampler
...
def renew_pipeline_sampler(pipe_line, **kwargs):
    ...
    static_tree = kwargs.get("static_tree", False)
    static_tree_plus = kwargs.get("static_tree_plus", False)
    if static_tree:
        if static_tree_plus:
            # pipe_line.model.__class__ = sjd_pp_sampler(pipe_line.model.__class__)
            pipe_line.model.__class__ = renew_static_tree_sampler_plus(pipe_line.model.__class__)
        else:
            pipe_line.model.__class__ = renew_static_tree_sampler(pipe_line.model.__class__)
    else:
        pipe_line.model.__class__ = renew_sampler(pipe_line.model.__class__)
    pipe_line.model._init_new_params(**kwargs)
    if static_tree:
        pipe_line.model.model.__class__ = renew_static_tree_backbone(pipe_line.model.model.__class__)
    else:
