...
from scheduler.logit_processor_3dim import SequenceSegmentDecomposer, get_double_cfg_input_ids, \
    check_eol_in_multitokens, get_eol_in_multitokens, get_eol_in_multitokens_with_cache_position

from scheduler.jacobi_iteration_lumina_mgpt import renew_sampler, renew_backbone, renew_static_tree_sampler
    ...
    class EOLLogitProcessor3d(model_class, LogitsProcessor):
        ...
        def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
            ...
            for batch_id in range(input_ids.shape[0]):
                ...
                pad_eol_len = 1
                if not hasattr(self, "cache_position") or self.cache_position is None:
                    ...
                    new_logit_token_len = self.cache_position.unique().shape[0]
                    ...
                    if is_new_seq_ids_containing_eol:
                        ...
                        batch_scores, eol_position_ids = get_eol_in_multitokens_with_cache_position(
                            batch_scores, self.eol_token, len(tokens), 
                            new_logit_token_len, self.width + pad_eol_len, self.cache_position,
                            min_dtype=min_dtype,
                        )
                    ...
                    if is_new_seq_ids_containing_eof:
                        batch_scores, eol_position_ids = get_eol_in_multitokens_with_cache_position(
                            batch_scores, self.eof_token, len(tokens), 
                            new_logit_token_len, 
                            (self.width + pad_eol_len) * self.height + 1, self.cache_position,
                            min_dtype=min_dtype,
                        )
                    ...
                    if is_new_seq_ids_containing_eoi:
                        batch_scores, eol_position_ids = get_eol_in_multitokens_with_cache_position(
                            batch_scores, self.eoi_token, len(tokens), 
                            new_logit_token_len, 
                            (self.width + pad_eol_len) * self.height + 2, self.cache_position,
                            min_dtype=min_dtype,
                        )
                    ...
                    if is_new_seq_ids_containing_eos:
                        batch_scores, eol_position_ids = get_eol_in_multitokens_with_cache_position(
                            batch_scores, self.eos_token, len(tokens), 
                            new_logit_token_len, 
                            (self.width + pad_eol_len) * self.height + 3, self.cache_position,
                            min_dtype=min_dtype,
                        )
                    
                    if is_new_seq_ids_containing_larger_eos:
                        max_len = (self.width + pad_eol_len) * self.height + 3 + self.offset_cache[batch_id] 
                        # larger_eos_start_idx = (
                        #     (self.width + pad_eol_len) * self.height + 3 - len(tokens)
                        # )
                        larger_eos_start_idx = torch.where(max_len <= self.cache_position)[0]
                        # batch_scores[ larger_eos_start_idx:, :] = min_dtype # 
                        # batch_scores[ larger_eos_start_idx:, self.pad_token ] = 0
                        batch_scores[ larger_eos_start_idx, :] = min_dtype
                        batch_scores[ larger_eos_start_idx, self.pad_token ] = 0
...
def renew_static_tree_sampler_forward(model_class):
    class JacobiModel(model_class):
        def _init_new_params(
            self, 
            *args, 
...
def renew_solver(model, processor, **jacobi_param_dict):
    ...
    fsp_input_ids = jacobi_param_dict.pop('fsp_input_ids') if 'fsp_input_ids' in jacobi_param_dict else None
    ...
    static_tree = jacobi_param_dict.get("static_tree", False)
    ...
    if static_tree:
        model.__class__ = renew_static_tree_sampler(model.__class__)
    else:
        model.__class__ = renew_sampler(model.__class__)
    model._init_new_params(**jacobi_param_dict)
    if static_tree:
        model.__class__ = renew_static_tree_sampler_forward(model.__class__)
    else:
    ...
    model.fsp_input_ids = fsp_input_ids
